...
Blockly.JavaScript.setter = function() {
  ...
  var propName = this.getFieldValue("PROP");                    // e.g. Text
  var compName = this.getFieldValue("COMPONENT_SELECTOR");   // e.g., Label1
...
this.domToMutation = function(xmlElement) {
  ...
  // Use the global shim if the workspace fails us
  var db = (this.workspace && this.workspace.getComponentDatabase) ? 
           this.workspace.getComponentDatabase() : Blockly.ComponentDatabase;
           
  var event = db.getEventForType(this.componentType, this.eventName);
  if (event) {
    this.eventParameters = event.parameters || [];
  }
  
  // App Inventor's blockly-all logic usually needs this call to redraw the block
